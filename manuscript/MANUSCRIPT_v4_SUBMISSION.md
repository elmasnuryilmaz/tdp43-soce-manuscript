...
Independent cohorts were analysed identically: Alzheimer's disease (GSE125583, fusiform gyrus, 219 cases/70 controls), Parkinson's disease (GSE68719, BA9, 29/44) and multiple sclerosis (GSE138614, white matter, 10 MS/5 control donors, 98 samples; GSE123496, five brain regions, 5/5 donors). GSE138614 is white matter, whereas the ALS, Alzheimer's and Parkinson's comparisons are predominantly grey matter; the tissue compartment therefore differs and is not controlled for. For multiple sclerosis, because TRPC1 is expressed in oligodendrocytes and oligodendrocyte precursors, two additional controls were applied: analysis restricted to normal-appearing white matter, in which myelin markers are preserved, and regression of TRPC1 on MBP, PLP1 and GFAP with testing of the residuals. Several GSE138614 samples come from the same donor, so the lesion-type and normal-appearing white matter comparisons were run both at sample level and after averaging within donors; the donor-level values are the ones reported.
...
Viability was measured at 24 h and 48 h with the WST-1 assay, normalised to the mean of the control group at the same time point (n = 4 wells).
...
Two negative results follow. First, the prediction from isoform-level testing that *STIM1* produces a premature-termination-codon-bearing, NMD-sensitive isoform was **not supported**: in the four-condition analysis the interaction was −0.325 (p = 0.317, q = 0.508), in the opposite direction, and we withdraw that inference. Second, in the same four-condition analysis none of the four Ca²⁺ panels showed collective NMD sensitivity (one-sided Mann–Whitney p = 0.44, 0.91, 0.96 and 0.91 for Tiers 1–4), and the sixteen positive-control genes were not NMD-sensitive as a group either (p = 0.62; Supplementary Table S10b), consistent with the absence of cryptic events in the core machinery.
...
The corrected analysis therefore identifies moderate candidate gradients in *STIM1* and *STIM2* but does not establish a direct APA event in any core SOCE gene. No index change of 0.05 or more was found for *ORAI1–3*, *TRPC1*, *SARAF*, *STIMATE*, *CBARP* or the SERCA genes, and the two units of those genes whose intervals exclude zero are negligible in size (*SARAF* terminal exon −0.036, *ORAI2* terminal exon +0.012). Two units outside the core panel are of comparable size to the *STIM* gradients and likewise exclude zero — the *ITPR3* terminal exon (−0.152) and the *MICU3* terminal exon (+0.097), with *ITPR1* at −0.055 — so the ER-release and mitochondrial-uptake arms show the same kind of candidate gradient as *STIM1* and *STIM2* rather than being spared. Because the positive control was no longer robust in this model after correction, absence of a signal should not be interpreted as evidence that these genes are definitively spared.
...
The corrected alternative-polyadenylation analysis is coverage-based and does not localise poly(A) sites; 3′-end sequencing would be required for that purpose. Excluding reference skips reduced the *STMN2* positive control in SH-SY5Y from a near-complete shift to +0.145 (bootstrap 95% CI +0.085 to +0.205), and the control then falls just below the pre-specified depth filter, so the negative result in that model rests on an assay whose sensitivity could not be demonstrated there. Across the four comparisons in which the corrected analysis could be run, the assay's own positive control is informative in the two neuronal models — the iPSC-derived motor neurons and NSC34 — and silent in SH-SY5Y and in the C2C12 myoblast line; the iPSC-derived motor neurons have only two replicates per group, so the interval around their working control is coarse. The analysis identified moderate candidate gradients in *STIM1* and *STIM2* in SH-SY5Y, neither of which was confirmed in another comparison, and no large APA shift in the core SOCE machinery in any comparison. Sparse windows remain a sensitivity limit, so the negative result is not definitive evidence of sparing.
...
**Table 2.** Robust splicing events in core SOCE machinery genes in the primary SH-SY5Y model, with bootstrap confidence intervals. The *CBARP* events, which lie outside the core panel, are in Supplementary Table S3.
...
**S6.** Cryptic positive controls, the sixteen literature genes, and the dataset × gene recovery matrix.
...
**S11.** Depth-qualified intronic polyadenylation and 3′UTR usage estimates from the corrected analysis in all four comparisons; these are coverage gradients, not direct poly(A)-site calls.
