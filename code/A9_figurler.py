#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A9 — Disk analizleri için şekiller (Figure 7-9). Etiketler İngilizcedir."""
import os, sys, glob
...
OUT = f"{D}/sonuclar"; FIG = f"{D}/figurler"; os.makedirs(FIG, exist_ok=True)
...
kaydet(fig, "Figure7_cryptic_discovery_and_NMD")
...
kaydet(fig, "Figure8_NYGC_cryptic_STMN2")
...
ax.set_title("A · Cryptic calls are TDP-43-specific", loc="left")
...
if len(st) == 2:
    ...
    for j in range(len(kd)):
        # The corrected analysis has one KD replicate with zero 3′-window
        # coverage; retain it in the panel as missing rather than implying a
        # measured terminal depth.
        if d3k[j] == 0:
            d3k[j] = np.nan
    ...
    ax.legend(frameon=False, fontsize=8)
    ax.set_title("C · Corrected coverage across STMN2 intron 2\n(one KD 3′ window missing)",
                 loc="left")
kaydet(fig, "Figure9_specificity_and_APA")
