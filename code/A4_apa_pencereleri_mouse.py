...
HEDEF = {g.capitalize() for g in (set(tier4) | set(POS))}

ex = pd.read_csv(f"{REF}/gencode.vM25.annotation.exons.txt.gz", sep="\t")
...
bed.to_csv(f"{ND}/kod/apa_pencereleri_mouse.bed", sep="\t", header=False, index=False)
