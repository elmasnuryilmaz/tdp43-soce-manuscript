#!/usr/bin/env python3
"""Manuscript figures (English labels)."""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np
import pandas as pd
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from matplotlib.patches import Patch
from importlib.machinery import SourceFileLoader

core = SourceFileLoader("core", os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                             "01_rmats_core.py")).load_module()
TAB = os.path.join(core.OUT, "03_TABLOLAR")
FIG = os.path.join(core.OUT, "02_FIGURLER")
os.makedirs(FIG, exist_ok=True)

plt.rcParams.update({
    "font.family": "DejaVu Sans", "font.size": 9,
    "axes.spines.top": False, "axes.spines.right": False,
    "axes.titlesize": 10, "axes.titleweight": "bold",
    "figure.dpi": 150, "savefig.dpi": 300, "savefig.bbox": "tight",
})
KD_C, CT_C, ACC, WARN = "#c0392b", "#2c6fa8", "#0d6259", "#b07d0a"

# ================================================================ FIG 1: TRPC1
fig, axes = plt.subplots(1, 3, figsize=(12.6, 3.6),
                         gridspec_kw={"width_ratios": [1, 1.15, 1.3], "wspace": 0.32})

# --- 1A: replika duzeyi PSI
ax = axes[0]
kd = [1.000, 0.778, 1.000]; ct = [1.000, 1.000, 0.455]
ct_x = [-0.08, 0.05, 0.18]          # ust uste binen noktalari ayirmak icin jitter
kd_x = [0.82, 0.95, 1.08]
ax.scatter(ct_x, ct, s=70, color=CT_C, zorder=3, label="Control")
ax.scatter(kd_x, kd, s=70, color=KD_C, zorder=3, label="TDP-43 KD")
ax.hlines(np.mean(ct), -0.15, 0.25, color=CT_C, lw=2)
ax.hlines(np.mean(kd), 0.75, 1.15, color=KD_C, lw=2)
for xs, v, n in [(ct_x, ct, ["2/0", "10/0", "10/6"]),
                 (kd_x, kd, ["25/0", "7/1", "28/0"])]:
    for xi, yi, ni in zip(xs, v, n):
        ax.annotate(ni, (xi, yi), textcoords="offset points", xytext=(0, 11),
                    fontsize=7, color="#555", ha="center")
ax.set_xlim(-0.4, 1.45); ax.set_ylim(0.3, 1.12)
ax.set_xticks([0.05, 0.95]); ax.set_xticklabels(["Control", "TDP-43 KD"])
ax.set_ylabel("PSI (inclusion level)")
ax.set_title("A · Replicate-level PSI", loc="left")
ax.text(0.02, 0.05, "labels: IJC/SJC", transform=ax.transAxes, fontsize=7, color="#777")

# --- 1B: bootstrap dagilimi
ax = axes[1]
df = core.load_event("GSE296712_SHSY5Y", "SE", "JC")
df["gene_up"] = df["geneSymbol"].str.upper()
r = df[(df.gene_up == "TRPC1") & (df.ID == 33987)].iloc[0]
rng = np.random.default_rng(42)
il, sl = r["IncFormLen"], r["SkipFormLen"]
kdi = np.array(r["IJC_SAMPLE_1_list"]); kds = np.array(r["SJC_SAMPLE_1_list"])
cti = np.array(r["IJC_SAMPLE_2_list"]); cts = np.array(r["SJC_SAMPLE_2_list"])
boot = []
for _ in range(20000):
    a = rng.integers(0, 3, 3); b = rng.integers(0, 3, 3)
    p1 = core.psi_from_counts(kdi[a].sum(), kds[a].sum(), il, sl)
    p2 = core.psi_from_counts(cti[b].sum(), cts[b].sum(), il, sl)
    if not (np.isnan(p1) or np.isnan(p2)):
        boot.append(p1 - p2)
boot = np.array(boot)
lo, hi = np.percentile(boot, [2.5, 97.5])
ax.hist(boot, bins=60, color="#b8c9c7", edgecolor="none")
ax.axvline(0, color="#333", lw=1.2, ls="--")
ax.axvline(0.108, color=KD_C, lw=2, label="rMATS ΔPSI = +0.108")
ax.axvspan(lo, hi, color=ACC, alpha=0.14)
ax.set_xlabel("bootstrap ΔPSI"); ax.set_ylabel("frequency")
ax.set_title("B · Bootstrap 95% CI spans zero", loc="left")
ax.legend(fontsize=7, frameon=False, loc="upper left")
ax.annotate(f"95% CI\n[{lo:+.2f}, {hi:+.2f}]", xy=(0.62, 0.62), xycoords="axes fraction",
            fontsize=8, color=ACC, weight="bold")

# --- 1C: birini disarida birak
ax = axes[2]
loo = core.leave_one_out_dpsi(r)
labs = [k.replace("_cikarildi", "").replace("_", " ") for k, _ in loo]
vals = [v for _, v in loo]
cols = [KD_C if k.startswith("KD") else CT_C for k, _ in loo]
y = np.arange(len(vals))
ax.barh(y, vals, color=cols, height=0.62)
ax.axvline(0, color="#333", lw=1)
ax.axvline(0.108, color="#999", lw=1, ls=":")
ax.set_yticks(y); ax.set_yticklabels(labs, fontsize=8)
ax.invert_yaxis(); ax.set_xlabel("ΔPSI with that replicate removed")
ax.set_title("C · One replicate reverses the sign", loc="left")
ax.annotate("removing control rep 3\nreverses the sign of ΔPSI", xy=(-0.074, 5),
            xytext=(0.02, 0.58), textcoords="axes fraction", ha="left",
            fontsize=7.5, color=WARN, weight="bold",
            arrowprops=dict(arrowstyle="->", color=WARN, lw=1))
fig.suptitle("Figure 1 · The TRPC1 skipped-exon call does not survive replicate-level "
             "inspection (SH-SY5Y; seven skipping reads in total)", y=1.04, fontsize=10.5, weight="bold")
fig.savefig(os.path.join(FIG, "Figure1_TRPC1_robustness.png"))
fig.savefig(os.path.join(FIG, "Figure1_TRPC1_robustness.pdf"))
plt.close(fig)
print("Figure 1 written")

# ================================================================ FIG 2: stokiyometri
S = pd.read_csv(os.path.join(TAB, "SOCE_mutlak_stokiyometri.tsv"), sep="\t")
S = S[S.durum.isna() | (S.durum == "")]
fams = ["ORAI (CRAC kanali)", "SERCA (ER'ye Ca geri alimi)", "STIM (ER Ca sensoru)",
        "SOCE duzenleyicileri", "Mitokondriyal Ca alimi"]
TITLES = {"ORAI (CRAC kanali)": "ORAI (CRAC channel)",
          "SERCA (ER'ye Ca geri alimi)": "SERCA (Ca²⁺ re-uptake into ER)",
          "STIM (ER Ca sensoru)": "STIM (ER Ca²⁺ sensor)",
          "SOCE duzenleyicileri": "SOCE regulators",
          "Mitokondriyal Ca alimi": "Mitochondrial Ca²⁺ uptake"}
fig, axes = plt.subplots(1, 5, figsize=(15, 3.6))
RTQ = {"TRPC1", "STIM1", "ORAI1", "ATP2A3"}
for ax, fam in zip(axes, fams):
    sub = S[S.aile == fam].sort_values("CPM_kontrol", ascending=False)
    sub = sub[sub.CPM_kontrol.notna()]
    x = np.arange(len(sub)); w = 0.38
    ax.bar(x - w/2, sub.CPM_kontrol, w, color=CT_C, label="Control")
    ax.bar(x + w/2, sub.CPM_KD, w, color=KD_C, label="TDP-43 KD")
    labs = []
    for g in sub.gen:
        labs.append(f"$\\bf{{{g}}}$*" if g in RTQ else g)
    ax.set_xticks(x); ax.set_xticklabels(labs, rotation=45, ha="right", fontsize=8)
    ax.set_title(TITLES[fam], loc="left", fontsize=9)
    if ax is axes[0]:
        ax.set_ylabel("CPM (absolute abundance)")
    net = sub.CPM_KD.sum() - sub.CPM_kontrol.sum()
    pct = 100 * net / sub.CPM_kontrol.sum()
    ax.annotate(f"family net: {pct:+.1f}%", xy=(0.97, 0.97), xycoords="axes fraction",
                ha="right", va="top", fontsize=8.5, weight="bold",
                color=(KD_C if net < 0 else ACC))
fig.legend(handles=[Patch(facecolor=CT_C, label="Control"),
                    Patch(facecolor=KD_C, label="TDP-43 KD")],
           loc="lower center", ncol=2, frameon=False, fontsize=9,
           bbox_to_anchor=(0.5, -0.16))
fig.suptitle("Figure 2 · Absolute stoichiometry of the SOCE machinery in SH-SY5Y. "
             "RT-qPCR targets (*) are minor isoforms; the dominant isoforms fall",
             y=1.06, fontsize=10.5, weight="bold")
fig.savefig(os.path.join(FIG, "Figure2_stoichiometry.png"))
fig.savefig(os.path.join(FIG, "Figure2_stoichiometry.pdf"))
plt.close(fig)
print("Figure 2 written")

# ================================================================ FIG 3: guc
P = pd.read_csv(os.path.join(TAB, "guc_simulasyonu.tsv"), sep="\t")
fig, ax = plt.subplots(figsize=(5.6, 3.8))
dps = [0.05, 0.10, 0.15, 0.20, 0.30]
cmap = plt.cm.viridis(np.linspace(0.15, 0.85, len(P)))
for (i, row), c in zip(P.iterrows(), cmap):
    ys = [row[f"dPSI_{d:.2f}"] for d in dps]
    ax.plot(dps, ys, "o-", color=c, lw=2, ms=5, label=f"{int(row.derinlik)} reads/sample")
ax.axhline(0.8, color="#333", ls="--", lw=1)
ax.annotate("80% power", xy=(0.055, 0.82), fontsize=8)
ax.axvline(0.10, color=WARN, ls=":", lw=1.5)
ax.annotate("conventional\n|ΔPSI| ≥ 0.10 threshold", xy=(0.105, 0.42), fontsize=8, color=WARN)
ax.set_xlabel("true ΔPSI"); ax.set_ylabel("detection power")
ax.set_ylim(0, 1.02)
ax.set_title("Figure 3 · Detection power in a 3 + 3 design\nthe TRPC1 event sat at ~10 reads per sample",
             loc="left")
ax.legend(fontsize=7.5, frameon=False, loc="upper left")
fig.savefig(os.path.join(FIG, "Figure3_power.png"))
fig.savefig(os.path.join(FIG, "Figure3_power.pdf"))
plt.close(fig)
print("Figure 3 written")

# ================================================================ FIG 4: saglam adaylar
B = pd.read_csv(os.path.join(TAB, "SOCE_izoform_bootstrap_GA.tsv"), sep="\t")
B = B[B.veri_seti == "GSE296712_SHSY5Y"].copy()
B["etiket"] = B.gen + " (" + B.olay + ")"
B = B.sort_values("dPSI")
fig, ax = plt.subplots(figsize=(6.4, 3.2))
y = np.arange(len(B))
for i, (_, r) in enumerate(B.iterrows()):
    good = r.sifir_GA_icinde == "hayir"
    ax.plot([r.GA_alt, r.GA_ust], [i, i], color=(ACC if good else "#bbb"), lw=3,
            solid_capstyle="round")
    ax.plot(r.dPSI, i, "o", ms=8, color=(ACC if good else "#999"),
            markeredgecolor="white", markeredgewidth=1.2, zorder=3)
# TRPC1'i referans olarak ekle
ax.plot([-0.093, 0.522], [len(B), len(B)], color=KD_C, lw=3, alpha=.55,
        solid_capstyle="round")
ax.plot(0.108, len(B), "o", ms=8, color=KD_C, markeredgecolor="white", markeredgewidth=1.2)
labels = list(B.etiket) + ["TRPC1 (SE) — the original candidate"]
ax.axvline(0, color="#333", lw=1, ls="--")
ax.set_yticks(list(y) + [len(B)]); ax.set_yticklabels(labels, fontsize=8.5)
ax.get_yticklabels()[-1].set_color(KD_C)
ax.get_yticklabels()[-1].set_weight("bold")
ax.set_xlabel("ΔPSI (bootstrap 95% CI)")
ax.set_title("Figure 4 · SOCE splicing candidates in SH-SY5Y\ngreen = confidence interval excludes zero",
             loc="left")
fig.savefig(os.path.join(FIG, "Figure4_robust_candidates.png"))
fig.savefig(os.path.join(FIG, "Figure4_robust_candidates.pdf"))
plt.close(fig)
print("Figure 4 written")
print("\nFigures ->", FIG)
